    ...
    def get_direct_irradiance_ratio(node: object, sensortype: object, called_L2: bool = False) -> object:
        ...
        for n in range(n_bin):
            ...
            if np.isnan(direct).any():
                logging.warning("direct contains NaN values at: %s" % wvl[np.isnan(direct)[n]])

            if np.isnan(diffuse).any():
                logging.warning("diffuse contains NaN values at: %s" % wvl[np.isnan(diffuse)[n]])

            if np.isnan(irr_direct).any():
                logging.warning("irr_direct contains NaN values at: %s" % wvl[np.isnan(irr_direct)[n]])

            if np.isnan(irr_diffuse).any():
                logging.warning("irr_diffuse contains NaN values at: %s" % wvl[np.isnan(irr_diffuse)[n]])

            if np.isnan(irr_env).any():
                logging.warning("irr_env contains NaN values at: %s" % wvl[np.isnan(irr_env)[n]])

            if np.isnan(solar_zenith).any():
                logging.warning("solar_zenith contains NaN values at: %s" % wvl[np.isnan(solar_zenith)[n]])
