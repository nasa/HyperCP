...
class FullCalibrationFile:
    ...
    # Reads a full calibration file and generates calibration data
    def read(self, fname):
        (_, filename) = os.path.split(fname.name)
        self.name = filename
        print(filename)
        line_number = 0
        results = {}       
        keys_to_find = ['[VERSION]', '[CALDATE]', '[CALLAB]', '[USER]', '[LAMP_ID]', '[PANEL_ID]', '[DEVICE]', 
                        '[LAMP_CCT]', '[LAMPDATA]', '[PANELDATA]', '[AMBIENT_TEMP]', '[CALDATA]' ]

        with open(fname.name) as f:
            for line in f:
                line_number += 1
        ...
        file = open(fname.name)
        content = file.readlines()      
        self.version = content[results['[VERSION]']]
        self.caldate = content[results['[CALDATE]']]
        self.callab = content[results['[CALLAB]']]
        self.user = content[results['[USER]']]
        self.lampid = content[results['[LAMP_ID]']]
        self.panelid = content[results['[PANEL_ID]']]
        self.id = content[results['[DEVICE]']]
        self.lampcct = content[results['[LAMP_CCT]']]
        self.temperature = content[results['[AMBIENT_TEMP]']]
        file.close()

        print("Test full characterization")
        data = pd.read_csv(fname.name, skiprows=results['[CALDATA]'], nrows=256, header=None, sep=r'\s+')
        # self.lampdata = pd.read_csv(fname.name, skiprows=results['[LAMPDATA]'], nrows=256, header=None, sep=r'\s+')
        # self.paneldata = pd.read_csv(fname.name, skiprows=results['[PANELDATA]'], nrows=256, header=None, sep=r'\s+')
        print (data)
        exit()
