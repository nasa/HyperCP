...
''' Morel et al. (2011) BRDF correction
    With OC4ME Chl inversion
'''
...
class M02:
    ...
    def __init__(self, bands, aot, wind, adf=None):
        ...
        LUT_OCP = xr.open_dataset(adf, group='BRDF/M02')
        self.LUT = xr.Dataset()
        ...
        self.LUT['foq'  ] = self.LUT['foq'  ].interp(wind_foq  =np.clip(wind,0,16))
        ...
        self.OC4MEcoeff = LUT_OCP.log10_coeff_LUT
    ...
    def forward(self, ds, normalized=False):
        ...
        log10chl_foq = np.clip(ds['log_chl'],
                           float(np.min(coeffs.foq.log_chl_foq)),
                           float(np.max(coeffs.foq.log_chl_foq)))



        forward_mod = coeffs.Rgoth * coeffs.foq.interp(wavelengths_FOQ=wave_foq).interp(log_chl_foq=log10chl_foq)
    ...
    def backward(self, ds, iter_brdf):
        ...
        # Compute the OC4ME "R"
        ds['log_chl_OC4ME_Ratio'] = np.log10(np.max([Rrs442, Rrs490, Rrs510], axis=0) / Rrs560)

        ds['log_chl'] = 0 * ds['log_chl_OC4ME_Ratio']
        # Apply OC4ME 5-degree polynomial
        for k, Ak in enumerate(self.OC4MEcoeff):
            ds['log_chl'] += Ak * (ds['log_chl_OC4ME_Ratio'] ** k)
