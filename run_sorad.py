...
PATH_HCP = f"{PATH_OS}/HyperCP_Sorad/HyperCP"  # Local path to HyperCP repository.
PATH_DATA = os.path.join(PATH_HCP,'Data','Sample_Data', PLATFORM)
##################################

# PATH_ANC = os.path.join(PATH_DATA,f'{CRUISE}_Ancillary.sb')
PATH_ANC = ''
...
def run_Command(fp_input_files):
    ...
    if MULTI_LEVEL:
        ...
        for from_level, to_level, ext in zip(FROM_LEVELS, TO_LEVELS, FILE_EXT):                        
            # inputFileBase = os.path.splitext(os.path.basename(fp_input_files))[0]     # 'FRM4SOC2_FICE22_NASA_20220715_120000_L1BQC'  
            inputFileBase = os.path.basename(fp_input_files).split("_" + from_level)[0] # 'FRM4SOC2_FICE22_NASA_20220715_120000, 1614100, etc.
